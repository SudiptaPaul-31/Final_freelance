...
const Resume = () => {
  ...
  const [selectedChart, setSelectedChart] = useState('Bar Chart'); // New state for chart selection
  ...
  const handleSubmit3 = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(JSON.parse(chartArray));
...
{chartData.length > 0 && (
          <div className="mt-6">
            <h3 className="text-xl font-semibold mb-2">Select Chart Type:</h3>
            <select
              value={selectedChart}
            ...
            <div className="mt-6">
              <h3 className="text-xl font-semibold mb-2">{selectedChart}</h3>
              {selectedChart === 'Bar Chart' ? renderBarChart(chartData) : renderPieChart(chartData)}
            </div>
